...
class AudioFeatures {
  ...
  const AudioFeatures({this.energy = 0, this.energyStd=0, this.centroid=0, this.bandwidth=0, this.rolloff=0, this.flatness=0, this.zcr=0, this.flux=0, this.sampleRate = 0, this.maxFlux=0, this.minFlux=0});
  ...
  AudioFeatures nomalize() {
    ...
    return AudioFeatures(
      ...
      energyStd: energyStd / max(energyStd, 1e-8),
      centroid: centroid / nyq,
      bandwidth: bandwidth / nyq,
      rolloff: rolloff / nyq,
      flatness: log1p(flatness * 100) / log1p(100),
      zcr: zcr,
      flux: clip((log1p(flux)-minLog)/(log1p(maxFlux)-minLog), 0, 1),
  ...
  @override
  String toString(){
    return "[sample rate: $sampleRate, energy: $energy, energyStd: $energyStd, centroid: $centroid, bandwidth: $bandwidth, rolloff: $rolloff, flatness: $flatness, zcr: $zcr, flux: $flux, minFlux: $minFlux, maxFlux: $maxFlux]";
  }
